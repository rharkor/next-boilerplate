...
export const envSetup = async (initDb: boolean = true) => {
  ...
  if (
    !fs.existsSync(appEnvPath) &&
    fs.existsSync(path.join(rootDir, "apps", "app", ".env.example"))
  ) {
    fs.copyFileSync(
      path.join(rootDir, "apps", "app", ".env.example"),
      appEnvPath
    );
  ...
  if (
    !fs.existsSync(landingEnvPath) &&
    fs.existsSync(path.join(rootDir, "apps", "landing", ".env.example"))
  ) {
    fs.copyFileSync(
      path.join(rootDir, "apps", "landing", ".env.example"),
      landingEnvPath
    );
  ...
  if (
    !fs.existsSync(cronEnvPath) &&
    fs.existsSync(path.join(rootDir, "apps", "cron", ".env.example"))
  ) {
    fs.copyFileSync(
      path.join(rootDir, "apps", "cron", ".env.example"),
      cronEnvPath
    );
